...
static str endpoint()
{	return Format ("http://%1/api/soap/mantisconnect.php", tracker_url);
}

static str soapaction (str name)
{	return Format ("http://%1/api/soap/mantisconnect.php/%2", tracker_url, name);
}

str issueStringRep (ns1__IssueData* data)
{	return Format ("[%1] Ticket #%2 (%3): reporter: \002%4\017, "
...
{	Print ("Requesting info on %1\n", idstr);
	...
	{	const char** err = soap_faultstring (soap);
		val = Format ("Error retrieving info for ticket %1: %2", idstr, str (*err));
		result = false;
	}
